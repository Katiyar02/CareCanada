                ...
                {hospitals.map((doctor) => (
                  <li className="doctor-card" key={doctor.id}>
                    ...
                    <div className="doctor-card-body">
                      <p><FontAwesomeIcon icon={faMapMarkerAlt} /> Distance: {doctor.distance} km</p>
                      <p><FontAwesomeIcon icon={faClock} /> Wait time: {doctor.Wait_time}</p>
                      <p><strong>Address:</strong> {doctor.hospital_name}</p>
                    </div>
                    <button className="btn btn-danger mt-2" onClick={() => { setSelectedDoctor(doctor); setShowModal(true); }}>Book Appointment</button>
                  </li>
                ))}
              </ul>
            </div>
